...
const tinhHoaDon = ()=>{
    ...
    const soKenhCaoCap = document.getElementById('soKenhCaoCap').value || 0;

    let bill = 0;
    if(loaiKhachHang === "residential"){
        bill = 20 + (7.5 * soKenhCaoCap);
    }else if(loaiKhachHang === "business"){
        bill = 75 + (50 * soKetNoi) + (50 * soKenhCaoCap);
    }

    document.getElementById("cableResult").innerHTML = `Hóa đơn cáp của khách hàng ${document.getElementById("maKhachHang").value là: $${bill.toFixed(2)}`
}